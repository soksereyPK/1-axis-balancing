...
int Tuning() {
  ...
  // Check if there is any serial data available, and if not, returns 0 and exits the function
  if (!Serial.available())  return 0;
  delay(2);

  // read one byte of serial data and assign to param
  char param = Serial.read();
  if (!Serial.available()) return 0;
  // read one byte of serial data and assigns to cmd
  char cmd = Serial.read();
  // clear serial buffer 
  Serial.flush();

  // switch statement that executed different cases based on the value of param
  switch (param) {
    case 'p':   // when param = p this case is executed 
      if (cmd == '+')    X1 += 1;  
      if (cmd == '-')    X1 -= 1;
      printValues();
      break;
